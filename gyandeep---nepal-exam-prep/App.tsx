...
import { UserProfile, QuizResult, QuizSettings, Question } from './types';
...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = React.useState(() => {
    ...
    // Route Guard Logic for initial load
    if (hash === 'admin') {
  ...
  // Track session time (1 minute intervals)
  React.useEffect(() => {
    if (!user) return;
    const interval = setInterval(() => {
      const updatedUser = {
        ...user,
        timeSpent: (user.timeSpent || 0) + 1,
      ...
      const userIndex = allUsers.findIndex((u: UserProfile) => u.email === user.email);
      if (userIndex >= 0) {
  ...
  const handleCategoriesChange = (newCats: any[]) => {
  ...
  if (currentQuizCategory && !quizResult) {
    return (
      <QuizEngine 
        ...
        onExit={() => setCurrentQuizCategory(null)} 
      />
    );
  }
    ...
    return (
      <Layout activeTab="quizzes" setActiveTab={handleSetActiveTab} onLogout={handleLogout} isAdmin={user.isAdmin}>
        <QuizSummary 
          ...
          onBack={() => { setQuizResult(null); setCurrentQuizCategory(null); handleSetActiveTab('dashboard'); }} 
        />
      </Layout>
    );
  }
    ...
    <Layout activeTab={activeTab} setActiveTab={handleSetActiveTab} onLogout={handleLogout} isAdmin={user.isAdmin}>
      {activeTab === 'dashboard' && !user.isAdmin && (
        <Dashboard 
          ...
          onSelectCategory={(cat) => { setCurrentQuizCategory(cat); handleSetActiveTab('quizzes'); }} 
        />
      )}
      ...
      {activeTab === 'quizzes' && !user.isAdmin && (
        <Dashboard 
          user={user} 
          categories={enabledCategories}
          onSelectCategory={setCurrentQuizCategory} 
        />
      )}
